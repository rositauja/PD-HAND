...
st.set_page_config(
    page_title="Admin Dashboard — PD-HAND",
    layout="wide",
...
# ── Step 5: Handle navbar page navigation after admin info exists ───────────────────────────────────────
go_page = st.query_params.get("go_page", None)

if go_page == "analytics":
    st.query_params.clear()
...
<div class="navbar">
    ...
    <div class="nav-tabs">
        <a class="nav-tab active" href="?go_page=dashboard&aid={admin_id}&aname={admin_name}" target="_self">📊 Dashboard</a>
...
<div class="main-container">
    <div class="page-header">
        ...
        <div class="page-subtitle">Anonymized overview of system usage and health screening results.</div>
    </div>
</div>
""", unsafe_allow_html=True)
...
high_likelihood_count = results.get("High Likelihood", 0)
low_likelihood_count = results.get("Low Likelihood", 0)
active_users = get_active_users_this_month()

# Calculate percentages
high_pct = round((high_likelihood_count / total_screenings * 100), 1) if total_screenings > 0 else 0
low_pct = round((low_likelihood_count / total_screenings * 100), 1) if total_screenings > 0 else 0
...
with col2:
    ...
    <div class="stat-card">
        ...
        <div class="stat-sub">All-time screenings</div>
        <div class="stat-icon">📋</div>
    </div>
    """, unsafe_allow_html=True)
...
with col3:
    ...
    <div class="stat-card">
        ...
        <div class="stat-value">{high_likelihood_count:,}</div>
        <div class="stat-sub">{high_pct}% of screenings</div>
...
with col4:
    st.markdown(f"""
    <div class="stat-card">
        <div class="stat-label">Low Likelihood</div>
        <div class="stat-value">{low_likelihood_count:,}</div>
        <div class="stat-sub">{low_pct}% of screenings</div>
        <div class="stat-icon">✓</div>
    </div>
    """, unsafe_allow_html=True)

with col5:
    avg_age = get_average_user_age()
    age_display = f"{avg_age} yrs" if avg_age > 0 else "—"
    st.markdown(f"""
    <div class="stat-card">
        <div class="stat-label">Average User Age</div>
        <div class="stat-value">{age_display}</div>
        <div class="stat-sub">Based on available data</div>
        <div class="stat-icon">👴</div>
    </div>
    """, unsafe_allow_html=True)
...
# ──── FIRST ROW: Screenings Chart + Result Ratio ────────────────────────
chart_col1, chart_col2 = st.columns([1.7, 1], gap="large")

with chart_col1:
    st.markdown("""
    <div class="card">
        <div class="card-title">Screenings — Last 30 Days</div>
        <div class="card-subtitle">Daily volume of handwriting screenings.</div>
    </div>
    """, unsafe_allow_html=True)
    
    screenings_30days = get_screenings_last_30_days()
    if screenings_30days:
        chart_df = pd.DataFrame(screenings_30days, columns=['Date', 'Screenings'])
        chart_df['Date'] = pd.to_datetime(chart_df['Date'])
        chart_df = chart_df.set_index('Date')
        st.line_chart(chart_df, color='#2E7D32', height=250)
    else:
        st.info("📊 No screening data in the last 30 days. Screenings will appear here as users complete them.")

with chart_col2:
    st.markdown("""
    <div class="card">
        <div class="card-title">Result Ratio</div>
        <div class="card-subtitle">High vs low likelihood outcomes.</div>
    </div>
    """, unsafe_allow_html=True)
    
    if total_screenings > 0:
        result_data = pd.DataFrame({
            'Result': ['Low Likelihood', 'High Likelihood'],
            'Count': [low_likelihood_count, high_likelihood_count]
        })
        st.bar_chart(result_data.set_index('Result'), color='#2E7D32', height=250)
    else:
        st.info("No screening data available")

# ──── SECOND ROW: New Users Chart + Recent Activity Table ──────────────
new_users_col, activity_col = st.columns([1, 1.7], gap="large")

with new_users_col:
    st.markdown("""
    <div class="card">
        <div class="card-title">New Users — Last 7 Days</div>
        <div class="card-subtitle">Daily sign-ups.</div>
    </div>
    """, unsafe_allow_html=True)
    
    new_users_7days = get_new_users_last_7_days()
    if new_users_7days:
        new_users_df = pd.DataFrame(new_users_7days, columns=['Date', 'Count'])
        new_users_df['Date'] = pd.to_datetime(new_users_df['Date'])
        new_users_df = new_users_df.set_index('Date')
        st.bar_chart(new_users_df, color='#2E7D32', height=250)
    else:
        st.info("No new user data available")

with activity_col:
    st.markdown("""<div class="card">
    <div class="card-title">Recent Activity</div>
    <div class="card-subtitle">Last 10 screenings. No personal identifiers are shown.</div>
    """, unsafe_allow_html=True)
    
    recent_data = get_recent_screenings(limit=10)
    
    if recent_data:
        table_html = "<table class=\"activity-table\"><thead><tr><th>DATE</th><th>RESULT</th><th>AGE</th><th>HAND</th><th>PATTERN</th></tr></thead><tbody>"
        
        for item in recent_data:
            result_class = "result-positive" if item['result'] == "High Likelihood" else "result-negative"
            result_label = "🔴 High Likelihood" if item['result'] == "High Likelihood" else "✅ Low Likelihood"
            ...
            table_html += f"<tr><td>{formatted_date}</td><td><span class=\"result-badge {result_class}\">{result_label}</span></td><td>{age}</td><td>{hand}</td><td>{pattern}</td></tr>"
        
        table_html += "</tbody></table></div>"
        st.markdown(table_html, unsafe_allow_html=True)
    else:
        st.markdown("</div>", unsafe_allow_html=True)
        st.info("No recent screening activity")
