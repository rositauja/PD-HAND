...
from core.database import create_tables, reset_password
...
if not st.session_state.get("otp_verified_direct"):
    st.switch_page("pages/12_Password_OTP_Direct.py")

# Initialize validation state
if "password_strength_direct" not in st.session_state:
    st.session_state.password_strength_direct = {
        "length": False,
        "number": False,
        "letter": False,
        "match": False
...
with col2:
    ...
    with st.form("reset_password_direct_form"):
        # New Password
        new_password = st.text_input(
            "New password",
            type="password",
            placeholder="At least 8 characters",
            key="new_password_direct",
        ...
        if new_password or confirm_password:
            ...
            st.markdown(f"""
                <div class="validation-item">
                    <div class="validation-checkbox {'checked' if st.session_state.password_strength_direct['length'] else ''}">
                        {'✓' if st.session_state.password_strength_direct['length'] else ''}
                    </div>
            ...
            st.markdown(f"""
                <div class="validation-item">
                    <div class="validation-checkbox {'checked' if st.session_state.password_strength_direct['number'] else ''}">
                        {'✓' if st.session_state.password_strength_direct['number'] else ''}
                    </div>
            ...
            st.markdown(f"""
                <div class="validation-item">
                    <div class="validation-checkbox {'checked' if st.session_state.password_strength_direct['letter'] else ''}">
                        {'✓' if st.session_state.password_strength_direct['letter'] else ''}
                    </div>
            ...
            st.markdown(f"""
                <div class="validation-item">
                    <div class="validation-checkbox {'checked' if st.session_state.password_strength_direct['match'] else ''}">
                        {'✓' if st.session_state.password_strength_direct['match'] else ''}
                    </div>
        ...
        if submitted:
            ...
            if not all(st.session_state.password_strength_direct.values()):
                ...
                if recovery_phone:
                    # Reset password (no phone change needed for direct path)
                    if reset_password(recovery_phone, new_password):
                        st.success("Password reset successfully! Redirecting to login...")
                        # Clear session
                        st.session_state.recovery_phone = None
                        st.session_state.otp_verified_direct = False
