...
from core.database import create_tables, verify_otp, get_otp_expiration_time, get_otp_attempts_remaining, generate_otp
...
# Check if user has set new phone
if not st.session_state.get("new_phone"):
...
with col2:
    ...
    st.markdown("""
        <div class="recovery-card">
            ...
            <p>We sent a 6-digit code to •••••••2030. Please enter it below.</p>
        </div>
    """, unsafe_allow_html=True)
    ...
    if not st.session_state.otp_generated:
        ...
        # Save to database
        generate_otp(st.session_state.new_phone, otp_code)
    ...
    with st.form("otp_form"):
        ...
        # Timer and Resend
        expiration = get_otp_expiration_time(st.session_state.new_phone)
        if expiration > 0:
        ...
        if verify_clicked:
            if len(otp_entered) != 6 or not otp_entered.isdigit():
                ...
                # Verify OTP
                if verify_otp(st.session_state.new_phone, otp_entered):
                    st.success("Phone number verified! Proceeding to set new password...")
                    st.session_state.otp_verified = True
                    time.sleep(1)
                    st.switch_page("pages/15_Password_Reset.py")
                else:
                    attempts = get_otp_attempts_remaining(st.session_state.new_phone)
                    if attempts > 0:
